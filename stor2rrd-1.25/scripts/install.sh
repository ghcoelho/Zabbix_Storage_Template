...
# creating ssh keys if they do not exist, needed for SVC/Storwize
...
if [ $COLLECTOR -eq 0 ]; then
  # create default web dir (it might be changed afterwards but there should be something at least
  if [ ! -f "$HOMELPAR/tmp/menu.txt" -a -f "$HOMELPAR/html/menu_default.txt" ]; then
    #place default menu.txt
    cp $HOMELPAR/html/menu_default.txt $HOMELPAR/tmp/menu.txt
  fi
  cp -R $HOMELPAR/html/* $HOMELPAR/www/ 2>/dev/null
fi
