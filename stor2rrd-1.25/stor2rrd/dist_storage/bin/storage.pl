...
# Foundation, Inc., 675 Mass Ave, Cambridge, MA 02139, USA.
  ...
  my $version_patch="$ENV{version_patch}";
  ...
  if ( ! defined($IO_MAX) || $IO_MAX eq '' || isdigit($IO_MAX) == 0 ) { 
    $IO_MAX = 50; # default IO max 
  }
  if ( ! defined($DATA_MAX) || $DATA_MAX eq '' || isdigit($DATA_MAX) == 0 ) { 
    $DATA_MAX = 500; # default data max  (kBytes)
  }
  if ( ! defined($RESPONSE_MAX) || $RESPONSE_MAX eq '' || isdigit($RESPONSE_MAX) == 0 ) { 
    $RESPONSE_MAX = 1; # default response time max  (ms)
  }
  if ( ! defined($CACHE_MAX) || $CACHE_MAX eq '' || isdigit($CACHE_MAX) == 0 ) { 
...
"#FAAFBE", "#357EC7", "#4AA02C", "#38ACEC", "#C0C0C0");
  ...
  my @drive_trans = ""; # drive name translation, must be global one
...
sub load_host  
{
    ...
    if ( $st_type =~ m/^HUS$/ ){
      if ( ! -f "$wrkdir/$host/HUS" ) {
        `touch "$wrkdir/$host/HUS"`;
      }
      LoadDataModule::load_data_hus_all ($host, $wrkdir, $webdir, $act_time, $STEP, $DEBUG, $no_time, $st_type);
    }
    if ( $st_type =~ m/^NETAPP$/ ){
      if ( ! -f "$wrkdir/$host/NETAPP" ) {
        `touch "$wrkdir/$host/NETAPP"`;
      }
      LoadDataModule::load_data_netapp_all ($host, $wrkdir, $webdir, $act_time, $STEP, $DEBUG, $no_time, $st_type);
    }
    if ( $st_type =~ m/^3PAR/ ){
      if ( ! -f "$wrkdir/$host/3PAR" ) {
        `touch "$wrkdir/$host/3PAR"`;
      }
      LoadDataModule::load_data_3par_all ($host, $wrkdir, $webdir, $act_time, $STEP, $DEBUG, $no_time, $st_type);
    }

    if ( $st_type =~ m/^VSPG$/ ){
      if ( ! -f "$wrkdir/$host/VSPG" ) {
        `touch "$wrkdir/$host/VSPG"`;
      }
      LoadDataModule::load_data_vspg_all ($host, $wrkdir, $webdir, $act_time, $STEP, $DEBUG, $no_time, $st_type);
    }
    ...
    my $ret = set_colors_initial($host,$type);
    if ( $ret == 2 ) {
      error ("Exiting $host as some core cfg file is missing: config.html || pool.cfg || volumes.cfg");
      return 1; # when does not exist config files config.html, pool.cfg and volumes.cfg
    }

    # storage independent so far ....
    $ret = Storcfg2html::cfg_ds8k ($host, $wrkdir, $webdir, $act_time, $DEBUG, $st_type);
    if ( $ret == 2 ) {
      error ("Exiting $host as some core cfg file is missing: config.html || pool.cfg || volumes.cfg");
      return 1; # when does not exist config files config.html, pool.cfg and volumes.cfg
    }
    ...
    if ( $st_type =~ m/^HUS$/ ) {
      draw_graphs_hus($host,$act_time,$st_type);
    }
    if ( $st_type =~ m/^NETAPP$/ ) {
      draw_graphs_netapp($host,$act_time,$st_type);
    }
    if ( $st_type =~ m/^3PAR$/ ) {
      draw_graphs_3par($host,$act_time,$st_type);
    }
    if ( $st_type =~ m/^VSPG$/ ) {
      draw_graphs_vspg($host,$act_time,$st_type);
    }
    print_storage_sys ($wrkdir,$host);

    if ( $once == 1  || $upgrade == 1 ) {
      global_volumes_top($storage,$st_type,$webdir,$act_time);
    }

    return 1;
}

sub draw_graphs_netapp
{
  my $host = shift;
  my $act_time = shift;
  my $st_type = shift;
  ...
  if ( -f "$wrkdir/$host/drive.cfg" ) {
    ...
    $item = "top";
    my $ret = volumes_in_top_all ($host,$type,$item,$st_type);
    if ( $ret == 2 ) {
...
sub draw_graphs_xiv 
{
  ...
  my $time_first = 1;
    ...
    # POOL
    # pool processing always after RANK due to *.first files, pool one are created based on already created for ranks
    # XIV pool data is based on Volume data
    #
    $type = "POOL";
...
sub draw_graphs_hus
{
  ...
  my $time_first = 1;
    ...
    my $size = (stat "$wrkdir/$host/pool.cfg")[7];
    if ( $size > 0  ) {
      # use POOL only if pools are in place, HUS do not need POOL, it can live with RG only
      # POOL
      #
      $type = "POOL";
      # here must be minimal time of the first data point from all files
    ...
    #$item = "sum_capacity";
    #draw_all_sum ($host,$type,$item,$st_type,$time_first);
    # --> not yet, we are not able to get size so far
    ...
    # PORT MAPPING #
    LoadDataModule::port_mapping($type,$wrkdir,$host,$act_time,$st_type,$DEBUG,$webdir);


  return 1;
}

sub draw_graphs_vspg
{
  my $host = shift;
  my $act_time = shift;
  my $st_type = shift;
...
sub draw_graph {
  ...
  if ( $item =~ m/^operating_rate$/ ) { $value_short= "%"; $value_long = "%"; } # HUS only  for drives
  ...
  if ( $item =~ m/^w_cache_hit$/){ $value_short= "%"; $value_long = "%"; $val = 1;}
  if ( $item =~ m/^r_cache_hit$/){ $value_short= "%"; $value_long = "%"; $val = 1;}
  ...
  my $gtype="AREA";
  if ( $item =~ m/operating_rate/ || $item =~ m/resp_t/ || $item =~ m/pprc_rt_/ || $item =~ m/^w_cache_hit$/ || $item =~ m/^r_cache_hit$/ ) {
    $gtype="LINE1";
  }
  ...
  foreach my $line (<$dir/*$suffix>) {
    ...
    if ( $st_type !~ m/NETAPP/ && $st_type !~ m/HUS/ && $type =~ m/RANK/ && $line !~ m/\/.*-P[0-9][0-9]*\.$suffix$/ ) {
    ...
    if ( $type =~ m/RANK/ &&  -f "$wrkdir/$host/mdisk.cfg" ) {
      ...
      foreach my $line (@mdisk_trans) {
        ...
        if ( defined($name) && ! $name eq '' && $id =~ m/^$lpar$/ ) {
          $lpar_space = $name;
          $rank_found = 1;
          $rank_id = $id;
          last;
    ...
    if ( $item =~ m/io/ ) {
      ...
      if ( $item =~ m/^sys$/ || $item =~ m/^compress$/ || $item =~ m/^operating_rate$/ ) {
        $cmd .= " GPRINT:$itemm${i}:AVERAGE:\\\"%3.1lf \\\"";
        $cmd .= " GPRINT:$itemm${i}:MAX:\\\"%3.1lf \\l\\\"";
        $cmd_print .= " PRINT:$itemm${i}:AVERAGE:\\\"%3.1lf $delimiter $lpar_space $delimiter $legend_heading $delimiter $color[$color_indx]\\\"";
        $cmd_print .= " PRINT:$itemm${i}:MAX:\\\" %3.1lf $delimiter $rrd_file_conv $delimiter $st_type\\\"";
      }
      else {
        $cmd .= " GPRINT:$itemm${i}:AVERAGE:\\\"%6.1lf \\\"";
        $cmd .= " GPRINT:$itemm${i}:MAX:\\\"%6.1lf \\l\\\"";
        $cmd_print .= " PRINT:$itemm${i}:AVERAGE:\\\"%6.1lf $delimiter $lpar_space $delimiter $legend_heading $delimiter $color[$color_indx]\\\"";
        $cmd_print .= " PRINT:$itemm${i}:MAX:\\\" %6.1lf $delimiter $rrd_file_conv $delimiter $st_type\\\"";
      }
    }
    if ( $item !~ m/operating_rate/ && $item !~ m/resp_t/ && $item !~ m/pprc_rt_/ && $item !~ m/^w_cache_hit$/ && $item !~ m/^r_cache_hit$/ ) {
      $gtype="STACK";
  ...
  if ( $i > 0 && ( $item =~ m/^sys$/ || $item =~ m/^compress$/ || $item =~ m/^operating_rate$/ )) {
    my $total = $i * 100;
...
sub draw_graph_pool {
  ...
  if ( $st_type =~ m/3PAR/ && $item_org =~ m/_b$/ ) {
    $rank_vol= "RANK"; # 3PAR back end data
  }
  if ( $st_type =~ m/HUS/ || $st_type =~ m/NETAPP/ || $st_type =~ m/VSPG/ ) {
    $searchstr = "";
    $rank_vol= "POOL"; 
  }
  # DS8K && 3PAR front-end stats are created in draw_all
  ...
  my @rank_list_unsorted = @rank_list;
  if ( $rank_list[0] =~ m/-/ && $st_type !~ m/NETAPP/ ) {
    # netapp 3857b20f-2fc7-4bbf-a023-5b040cf17099.rrd
    @rank_list = sort { (split '-', $a)[1] cmp (split '-', $b)[1] } @rank_list_unsorted;
  }
  else {
    @rank_list = sort @rank_list_unsorted;
  }
  ...
  foreach my $line  (@rank_list) {
    ...
    if ($print_now) {
      ...
      if ( $st_type =~ m/^3PAR$/ || $st_type =~ m/^DS8K$/ || $st_type =~ m/^SWIZ$/ ) {
	...
	$rrd_file_conv =~ s/\/RANK\/.*/\/POOL\/$print_now_pool_id\.rrd/;
      ...
      if ( $st_type =~ m/^DS5K$/ || $st_type =~ m/^XIV$/ || $st_type =~ m/3PAR/ ) {
	# DS8K stats from RANK mus be converted here to point to POOL to make working legend links
	$rrd_file_conv =~ s/\/VOLUME\/.*/\/POOL\/$print_now_pool_id\.rrd/;
      }
      #print "00 $rrd_file - $rrd_file_conv - $print_now_pool_id\n";

      $cmd .= " $gtype:pool${i}$color[$color_indx]:\\\"$pool_name_actual \\\"";
      if ( $item =~ m/resp_t/ )    { 
        $cmd .= " GPRINT:pool${i}:AVERAGE:\\\"%7.1lf \\\"";
        $cmd .= " GPRINT:pool${i}:MAX:\\\"%7.1lf \\l\\\"";
        $cmd .= " PRINT:pool${i}:AVERAGE:\\\"%7.1lf $delimiter $pool_name_actual $delimiter $legend_heading $delimiter $color[$color_indx]\\\"";
        ...
        $cmd .= " PRINT:pool${i}:AVERAGE:\\\"%7.0lf $delimiter $pool_name_actual $delimiter $legend_heading $delimiter $color[$color_indx]\\\"";
...
sub draw_graph_sum {
  ...
  if ( $item =~ m/^sum_io_total$/ ) { $value_short= "IOPS"; $value_long = "IO per second"; }
  if ( $item =~ m/^sum_data_total$/){ $value_short= "MB"; $value_long = "MBytes"; $val = 1024;}
  ...
  if ( $item =~ m/^write_pend$/ )   { $value_short= "%"; $value_long = "%"; }
  if ( $item =~ m/^clean_usage$/ )  { $value_short= "%"; $value_long = "%"; }
  if ( $item =~ m/^middle_usage$/ ) { $value_short= "%"; $value_long = "%"; }
  if ( $item =~ m/^phys_usage$/ )   { $value_short= "%"; $value_long = "%"; }


  # Why??? There should be 1024
  if ( $st_type =~ m/^DS8K$/ ) {
    if ( $item =~ m/^sum_data$/ || $item =~ m/^read$/ || $item =~ m/^write/ ) {
      $val = 1;
    }
  }
  ...
  my $type_real = $type; # real directory for file search --> XIV and DS5K use VOLUME for POOL sum_io and sum_data
  ...
  foreach my $line (<$wrkdir/$host/$type_real/*rrd>) {
    chomp($line);
    if ( $line !~ m/\.rrd$/ ) {
      next; # it does not end by .rrd suffix, "$ cannt be use in foreach <>
    }

    if ( $st_type !~ m/NETAPP/ && $st_type !~ m/HUS/ && $type =~ m/RANK/ && $line !~ m/\/.*-P[0-9][0-9]*\.rrd$/ && $st_type !~ m/VSPG/) {
      #workaround for cleaning up ranks without pool id (could hapened in the past by the error)
      next;
    }
    if ( $type =~ m/POOL/ ) {
      if ( $st_type !~ m/NETAPP/ && $st_type !~ m/HUS/ &&  $st_type !~ m/VSPG/ && $line !~ m/-cap\.rrd$/ && $st_type !~ m/NETAPP/ ) {
        #HUS have capacity in pool.rrd
    ...
    if ( $type =~ m/POOL/ && $line =~ m/-cap\.rrd$/ ) {
      if ( $item =~ m/^sum_io/ || $item =~ m/^sum_data/ ) {
        next; # skip *-cap.rrd files for sum_io and sum_data
      }
    }

    # for XIV and DS5K and sum_* select only POOL data files
    if ( $st_type =~ m/3PAR/ || $st_type =~ m/XIV/ || $st_type =~ m/DS5K/ ) {
      if ( $type =~ m/POOL/ ) {
        if ( $item =~ m/sum_io/ || $item =~ m/sum_data/ ) {
          my $line_base = basename($line);
          if ($line_base !~ m/.*-P.*\.rrd$/ ) {
            next; # it is not POOL data file
          }
        } 
      }
    }
    ...
    if ( $item =~ m/clean_usage/ ) {
      $cmd .= " DEF:$item${i}_r_u=\\\"$line\\\":clean_usage:AVERAGE";
      $cmd .= " CDEF:$itemm${i}_r=TIME,$time_first,LT,$item${i}_r_u,$item${i}_r_u,UN,UNKN,$item${i}_r_u,IF,IF";
      $cmd .= " CDEF:$itemm${i}_w=$item${i}_r_u,0,*";
    }
    if ( $item =~ m/middle_usage/ ) {
      $cmd .= " DEF:$item${i}_r_u=\\\"$line\\\":middle_usage:AVERAGE";
      $cmd .= " CDEF:$itemm${i}_r=TIME,$time_first,LT,$item${i}_r_u,$item${i}_r_u,UN,UNKN,$item${i}_r_u,IF,IF";
      $cmd .= " CDEF:$itemm${i}_w=$itemm${i}_r,0,*";
    }
    if ( $item =~ m/phys_usage/ ) {
      $cmd .= " DEF:$item${i}_r_u=\\\"$line\\\":phys_usage:AVERAGE";
      $cmd .= " CDEF:$itemm${i}_r=TIME,$time_first,LT,$item${i}_r_u,$item${i}_r_u,UN,UNKN,$item${i}_r_u,IF,IF";
      $cmd .= " CDEF:$itemm${i}_w=$itemm${i}_r,0,*";
    }
    if ( $item =~ m/write_pend/ ) {
      $cmd .= " DEF:$item${i}_r_u=\\\"$line\\\":write_pend:AVERAGE";
      $cmd .= " CDEF:$itemm${i}_r=TIME,$time_first,LT,$item${i}_r_u,$item${i}_r_u,UN,UNKN,$item${i}_r_u,IF,IF";
      $cmd .= " CDEF:$itemm${i}_w=$itemm${i}_r,0,*";
    }
    if ( $item =~ m/operating_rate/ ) {
      $cmd .= " DEF:$item${i}_r_u=\\\"$line\\\":operating_rate:AVERAGE";
      $cmd .= " CDEF:$itemm${i}_r=TIME,$time_first,LT,$item${i}_r_u,$item${i}_r_u,UN,UNKN,$item${i}_r_u,IF,IF";
      $cmd .= " CDEF:$itemm${i}_w=$itemm${i}_r,0,*";
    }
    if ( $item =~ m/sys/ ) {
      ...
      if ( $st_type =~ m/3PAR/ ) {
        $cmd .= " DEF:$item${i}_w_u=\\\"$line\\\":usr:AVERAGE";
      }
      else {
    ...
    if ( $item =~ m/sum_io_total/ ) {
      # DS5K does not have read/write details
      # To make it easier use _r and _w here
      $cmd .= " DEF:$item${i}_r_u=\\\"$line\\\":io_rate:AVERAGE";
      $cmd .= " CDEF:$item${i}_r=TIME,$time_first,LT,$item${i}_r_u,$item${i}_r_u,UN,0,$item${i}_r_u,IF,IF";
      $cmd .= " CDEF:$item${i}_w=$item${i}_r,0,*";
    }
    if ( $item =~ m/sum_data$/ ) {
  ...
  if ($item =~ m/^sys$/ || $item =~ m/^operating_rate$/ || $item =~ m/^write_pend$/ || $item =~ m/^clean_usage$/ || $item =~ m/^middle_usage$/ || $item =~ m/^phys_usage$/) {
    if ( $index_actual > 0 ) {
  ...
  if ( $item =~ m/sum_io_total/ || $item =~ m/sum_data_total/) {
    # DS5k only
    my $lpar_space = sprintf ("%-38s","Total");
    $cmd .= " $gtype:rsum${i}#00FF00:\\\"$lpar_space\\\"";
    $cmd .= " GPRINT:rsum${i}:AVERAGE:\\\"%9.0lf \\\"";
  }
  if ( $item =~ m/sum_capacity/ ) {
  ...
  if ( $item =~ m/operating_rate/ || $item =~ m/write_pend/ || $item =~ m/clean_usage/ || $item =~ m/middle_usage/ || $item =~ m/phys_usage/ ) {
    my $lpar_space = sprintf ("%-38s","usage");
    $cmd .= " LINE1:rsum${i}#FF3333:\\\"$lpar_space\\\"";
    $cmd .= " GPRINT:rsum${i}:AVERAGE:\\\"%7.2lf \\\"";
  }
  ...
  # print LINE read and write
  # DS5k sum_io_total && sum_data_total is skipped here
  if ( $item =~ m/sum_io$/ || $item =~ m/sum_data$/ ) {
  ...
  if ( $item =~ m/sys/ && $st_type !~ m/^HUS$/ && $st_type !~ m/^VSPG$/ && $st_type !~ m/^NETAPP$/ ) {
    my $lpar_space = sprintf ("%-38s","Compress");
    if ( $st_type =~ m/3PAR/ ) {
      $lpar_space = sprintf ("%-38s","CPU user");
    }
...
sub draw_graph_volume {
  ...
  if ( ! -f "$wrkdir/$host/$type/volumes.cfg" ) {
    error("$host: Volumes cfg file has been found (ignore after fresh install): $wrkdir/$host/$type/volumes.cfg ".__FILE__.":".__LINE__);
    return 0;
  }
...
sub get_pool_name
{
  ...
  foreach my $id (@pool_tr_table_id) {
    if ( $id eq $pool_id ) {
      if ( defined($pool_tr_table_name[$counter]) && $pool_tr_table_name[$counter] ne '' ) {
...
sub draw_graph_port_subsystem {
  ...
  if ( $item =~ m/^read_io$/ )   { $value_short= "IOPS"; $value_long = "IO per second"; }
  if ( $item =~ m/^write_io$/ )  { $value_short= "IOPS"; $value_long = "IO per second"; }
  if ( $item =~ m/^resp_t/ )   { $value_short= "ms"; $value_long = "ms"; }
  ...
  foreach my $line (<$wrkdir/$host/$type/*rrd>) {
    ...
    $cmd .= " CDEF:fcoe${i}=type${i},5,EQ,$itemm${i},0,IF";   # IPREP disky
    $cmd .= " CDEF:rcip${i}=type${i},6,EQ,$itemm${i},0,IF";   # RCIP  disky
    $i++;
  }
  ...
  if ( $st_type =~ m/3PAR/ ) {
    # FC0E 
    $index_actual = 0;
    $cmd .= " CDEF:fcoe=fcoe${index_actual}";
    $index_actual++;
    for (; $index_actual < $i; $index_actual++) { 
      $cmd .= ",fcoe${index_actual},+";
    }
    $cmd .= " $gtype:fcoe#FF4444:\\\"FCoE        \\\"";
    if ( $item =~ m/io/ ) {
      $cmd .= " GPRINT:fcoe:AVERAGE:\\\"%6.0lf \\\"";
      $cmd .= " GPRINT:fcoe:MAX:\\\"%6.0lf \\l\\\"";
    }
    else {
      $cmd .= " GPRINT:fcoe:AVERAGE:\\\"%6.1lf \\\"";
      $cmd .= " GPRINT:fcoe:MAX:\\\"%6.1lf \\l\\\"";
    }

    # RCIP
    $index_actual = 0;
    $cmd .= " CDEF:rcip=rcip${index_actual}";
    $index_actual++;
    for (; $index_actual < $i; $index_actual++) {
      $cmd .= ",rcip${index_actual},+";
    }
    $cmd .= " $gtype:rcip#FF4444:\\\"RCIP        \\\"";
    if ( $item =~ m/io/ ) {
      $cmd .= " GPRINT:rcip:AVERAGE:\\\"%6.0lf \\\"";
      $cmd .= " GPRINT:rcip:MAX:\\\"%6.0lf \\l\\\"";
    }
    else {
      $cmd .= " GPRINT:rcip:AVERAGE:\\\"%6.1lf \\\"";
      $cmd .= " GPRINT:rcip:MAX:\\\"%6.1lf \\l\\\"";
    }
...
sub find_real_data_start_pool
{
  my $rank_dir = shift;
  my $pool_id = shift;
  if ( isdigit($pool_id) == 1 && $st_type !~ m/VSPG/ ) {
...
sub find_real_data_start_all 
{
  ...
  foreach my $rrd_rank (<$rank_dir/*\.rrd>) {
...
sub volumes_in_top_all 
{
  ...
  my $ret = volumes_in_top    ("d",$host,$type,$st_type,$item);
  if ( $ret > 0 ) { 
    return $ret;
  }
  $ret = volumes_in_top    ("w",$host,$type,$st_type,$item);
  if ( $ret > 0 ) { 
    return $ret;
  }
  $ret = volumes_in_top    ("m",$host,$type,$st_type,$item);
  if ( $ret > 0 ) { 
    return $ret;
  }
  $ret = volumes_in_top    ("y",$host,$type,$st_type,$item);
  if ( $ret > 0 ) { 
    return $ret;
  }
  return 0;
...
sub volumes_in_top  {
  ...
  if ( $st_type =~ m/DS5K/ && -f "$wrkdir/$host/DS5K-v1" ) {
    @item_list = ("data_rate","io_rate","cache_hit");
    $no_of_items = 3; # number items in @item_list
  }
  if ( $st_type =~ m/DS5K/ && -f "$wrkdir/$host/DS5K-v2" ) {
    @item_list = ("data_rate","io_rate","resp_t","r_cache_hit","w_cache_hit","ssd_r_cache_hit");
    $no_of_items = 6; # number items in @item_list
  }
  if ( $st_type =~ m/3PAR/ || $st_type =~ m/XIV/ || $st_type =~ m/NETAPP/ ) {
  ...
  # read volume cfg files with grouped volumes
  if ( ! -f "$wrkdir/$host/$type/volumes.cfg" ) {
    error("$host: Volumes cfg file has been found : $wrkdir/$host/$type/volumes.cfg ".__FILE__.":".__LINE__);
    return 2;
  }
  open(FHR, "< $wrkdir/$host/$type/volumes.cfg") || error ("$host: file does not exists : $wrkdir/$host/$type/volumes.cfg ".__FILE__.":".__LINE__) && return 2;
  my @files = <FHR>;
  close (FHR);
  ...
  foreach my $volg (@files) {
    ...
    foreach my $file (@vol_array_work) {
      ...
      foreach my $item_act (@item_list) {
        ...
        # bulid RRDTool cmd
        $file_act =~ s/:/\\:/g; # in case there is ":" in the rrd file name, RRDTool then has a problem with parsing
  ...
  my $FH;
  open(FH, "> $tmp_file") || error ("Can't open $tmp_file : $! ".__FILE__.":".__LINE__) && return 1;
  print FH "$cmd\n";
  close (FH);
...
sub set_colors_initial
{
  ...
  my @color_save = "";
  if ( -f "$wrkdir/$host/$type/volumes.col" ) {
    open(FHC, "< $wrkdir/$host/$type/volumes.col") || error ("file does not exists : $wrkdir/$host/$type/volumes.col ".__FILE__.":".__LINE__) && return 1;
    @color_save = <FHC>;
    close (FHC);
  }
...
sub ishexa
{
  ...
  $digit_work =~ s/e//;
  $digit_work =~ s/\+//;
  $digit_work =~ s/\-//g; # to support UUID's 107d3696-f7ac-4db8-9adc-48208635e9a0
...
sub create_dir_structure 
{
  ...
  my $tmp_dir = shift;
    ...
    if ( $st_type =~ m/^HUS$/ ) {
      ...
      if (! -l "$wrkdir/$host/RG" ) {
        print "ln -s          : $wrkdir/$host/RANK $wrkdir/$host/RG\n" if $DEBUG ;
        symlink("$wrkdir/$host/RANK","$wrkdir/$host/RG") || die   "$act_time: Cannot ln -s $wrkdir/$host/RG: $!";
        LoadDataModule::touch();
      }
      if (! -d "$wrkdir/$host/HOST" ) {
        print "mkdir          : $wrkdir/$host/HOST\n" if $DEBUG ;
...
sub draw_graph_pool_controler {
  ...
  if ( $st_type =~ m/HUS/ || $st_type =~ m/NETAPP/ ) {
    $searchstr = "";
    $rank_vol= "VOLUME"; # HUS uses volumes
    #$rank_vol= "POOL"; # HUS uses pools
    # no no, controller info is only in volume.rrd
  }
...
sub once_a_day {
  ...
  if ( -f "$version_file-run" ) {
    return 1; # "-run" file is already created in this run but it must report every time (for every storage)
  }

  if ( ! -f $version_file ) {
    #error("version file er: $version_file does not exist, it should not happen, creating it");
    # --> no error, it is ok after upgrade
    `touch $version_file`; 
...
# prints out the Global Volume Top tables --> it runs once a daty (1st run after the midnight or after upgrade)

sub global_volumes_top 
{
  my $storage = shift;
  my $st_type = shift;
  my $webdir = shift;
  ...
  print "global vol top : $storage:$type:$name:avrg:d\n";
  volumes_top ($storage,$type,$name,"avrg","d",$st_type,$webdir,$act_time);
  print "global vol top : $storage:$type:$name:avrg:w\n";
  volumes_top ($storage,$type,$name,"avrg","w",$st_type,$webdir,$act_time);
  print "global vol top : $storage:$type:$name:avrg:m\n";
  volumes_top ($storage,$type,$name,"avrg","m",$st_type,$webdir,$act_time);
  print "global vol top : $storage:$type:$name:max:d\n";
  volumes_top ($storage,$type,$name,"max","d",$st_type,$webdir,$act_time);
  print "global vol top : $storage:$type:$name:max:w\n";
  volumes_top ($storage,$type,$name,"max","w",$st_type,$webdir,$act_time);
  print "global vol top : $storage:$type:$name:max:m\n";
  volumes_top ($storage,$type,$name,"max","m",$st_type,$webdir,$act_time);

  return 1;
}

# below code is copied and slightly modified from detail-graph-cgi.pl
sub volumes_top 
{
  my $host = shift;
  my $type = shift;
  ...
  my $out_file = "$webdir/glob-$type-$name-$item-$time.html-tmp";
  #"read","write","read_io","write_io","resp_t_r","resp_t_w","r_cache_hit","w_cache_hit";
  #DS5K-v1:"io_rate","data_rate","cache_hit";
  #DS5K-v2:"io_rate","data_rate","resp_t","r_cache_hit","w_cache_hit","ssd_r_cache_hit";
  ...
  if ( ! -f $out_file ) {
    open(FHT, "> $out_file") || error ("Can't open $out_file : $! ".__FILE__.":".__LINE__) && return 0;
    print FHT "<center>\n";
    if ( $time =~ m/^d/ ) {
      print FHT "last day (created on: $act_time)<br>\n";
    }
    if ( $time =~ m/^w/ ) {
      print FHT "last week without last day (created on: $act_time)<br>\n";
    }
    if ( $time =~ m/^m/ ) {
      print FHT "last month without last week (created on: $act_time)<br>\n";
    }
    print FHT "<table class=\"lparsearch tablesorter\"><thead><tr> \n
         <th class=\"sortable\" align=\"center\" valign=\"top\">&nbsp;&nbsp;&nbsp;Storage&nbsp;&nbsp;&nbsp;</th> \n
  ...
  foreach my $line (@lines) {
    ...
    $volume =~ s/=====space=====/ /g;
    #print "001 --$line: $volume:$item:$value \n";
    ...
    if ($volume_prev eq '' || $volume_prev !~ m/^$volume$/ ) {
      if (! $volume_prev eq '' ) {
        if ( $st_type =~ m/XIV/ || $st_type =~ m/NETAPP/ || $st_type =~ m/3PAR/ ) {
          print FHT "<td align=\"right\">$null&nbsp;&nbsp;&nbsp;</td><td align=\"right\">$null&nbsp;&nbsp;&nbsp;</td>";
        }
        print FHT "</tr>\n";
      }
      print FHT "<tr><td><a href=\"/stor2rrd-cgi/detail.sh?host=$storage&type=VOLUME&name=io&storage=$st_type&item=sum&gui=1&none=none\">$storage</a></td><td><a href=\"/stor2rrd-cgi/detail.sh?host=$host&type=VOLUME&name=$volume&storage=$st_type&none=none\">$volume</a></td>\n";
      $volume_prev = $volume;
      $item_no = 0;
      $data_rate = 0;
      $io_rate = 0;
    ...
    if ( $st_type =~ m/DS5K/ ) {
      if ( $item_no == 0 || $item_no == 1 ) {
        print FHT "    <td align=\"right\">$null&nbsp;&nbsp;&nbsp;</td>";
        print FHT "    <td align=\"right\">$null&nbsp;&nbsp;&nbsp;</td>";
      }
      if ( $item_no == 2 ) {
        print FHT "    <td align=\"right\">$null&nbsp;&nbsp;&nbsp;</td>";
      ...
      if ( ! -f "$wrkdir/$host/DS5K-v2" && $item_no > 2 ) {
        next; # ignore rest of DS5 stuff
      }
      if ( -f "$wrkdir/$host/DS5K-v2" && $item_no > 4 ) {
        next; # ignore rest of DS5 stuff for v2 (ssd cache)
      }
    }
    $item_no++;
    print FHT "<td align=\"right\" title=\"$item\">$value&nbsp;&nbsp;&nbsp;</td>";
    if ( $st_type =~ m/DS5K/ ) {
      next;
    }

    # get data/io rate from read/write values for != DS5k
    if ( $item_no == 1 ) {
      $data_rate = $value;
    }
    if ( $item_no == 2 ) {
      if ( isdigit($data_rate) && isdigit($value) ) {
        my $data_rate_tmp = $data_rate+$value;
        $data_rate = sprintf("%.1f",$data_rate_tmp);
        print FHT "<td align=\"right\" title=\"date_rate\">$data_rate&nbsp;&nbsp;&nbsp;</td>"
      }
      else {
        print FHT "<td align=\"right\" title=\"date_rate\">$null&nbsp;&nbsp;&nbsp;</td>"
      }
    } 
    if ( $item_no == 3 ) {
      $io_rate = $value;
    ...
    if ( $item_no == 4 ) {
      if ( isdigit($io_rate) && isdigit($value) ) {
        my $io_rate_tmp = $io_rate+$value;
        $io_rate = sprintf("%.1f",$io_rate_tmp);
        print FHT "<td align=\"right\" title=\"io_rate\">$io_rate&nbsp;&nbsp;&nbsp;</td>"
      }
      else {
        print FHT "<td align=\"right\" title=\"io_rate\">$null&nbsp;&nbsp;&nbsp;</td>"
      }
    } 
  }
