...
if ($putty) {
	...
	$scp = $pscp." -i $key $user\@$svc:/tmp/svc.config.backup.xml";
...
my @html_exclude_string = qw(&up; &dquot;);
if (defined $ENV{SVC_HTML_EXCLUDE_STRING}) {
	@html_exclude_string = split(/ /,$ENV{SVC_HTML_EXCLUDE_STRING}); 
} else {
	@html_exclude_string = qw(&up; &dquot;);
}
...
if ( defined $tmp_conf_file ) {
	if($debug) {&message("Config Filename: " . $dir . "/" . $tmp_conf_file)};
	if ( -r $dir . "/" . $tmp_conf_file ) {
		$prevcfg_ref = retrieve($dir . "/" . $tmp_conf_file);
		if ( ! defined $prevcfg_ref ) {
			&message("Retrieve data from file $tmp_conf_file failed.");
			system("$cmd_rm -f ".$dir."/".$tmp_conf_file );
			$fullcfgflg = 1;
		} elsif (! scalar(%{$prevcfg_ref}) ) {
			if($debug) { &message("Previous configuration data is not valid.")};
			system("$cmd_rm -f ".$dir."/".$tmp_conf_file );
			$fullcfgflg = 1;
		} elsif (! defined $prevcfg_ref->{'fullcfg_epoch'} ) {
			if($debug) { &message("Full Config Epoch is not find in previous data.")};
			$fullcfgflg = 1;
		} elsif ( $local_epoch - $prevcfg_ref->{'fullcfg_epoch'} >= $fullcfgint ) {
			$fullcfgflg = 1;
			if($debug) {
				&message("Full Config Time = ".localtime($prevcfg_ref->{'fullcfg_epoch'})." - epoch = ".$prevcfg_ref->{'fullcfg_epoch'});
				my $diff = $local_epoch - $prevcfg_ref->{'fullcfg_epoch'};
				&message("Full Config Interval = ".$diff);
	...
	$fullcfgflg = 1;
	if($debug) {&message("Config File is not present.")};
}
...
if ( $codevrmn >= 6040000 && $codevrmn <= 7010001 ) {
	...
	if ( $fullcfgflg ) {
		...
		if ($debug) { message("$ssh svcconfig backup"); }
		if ($putty) {
			system("$ssh svcconfig backup") == 0 or die("svcconfig.pl: ERROR: Command \"$ssh svcconfig backup\" failed.\n");
		} else {
			system("$ssh svcconfig backup 2>/dev/null >/dev/null") == 0 or die("svcconfig.pl: ERROR: Command \"$ssh svcconfig backup\" failed.\n");
		}

		if ($debug) { message("$scp $dir"); }
...
sub message {
	my $msg = shift;
	my $tm = localtime();
	print($tm." - INFO    - svcconfig.pl: ".$msg."\n");
}

sub warning {
	my ($msg,$rc) = @_;
	my $tm = localtime();
	print STDERR ($tm." - WARNING - svcconfig.pl: ".$msg."\n");
}

sub error {
	my ($msg,$rc) = @_;
	my $tm = localtime();
	print STDERR ($tm." - ERROR   - svcconfig.pl: ".$msg."\n");
}
...
sub CheckConfigFile {
	...
	my $tmpfile = $dir."/"."svc.config.backup.xml.check";
	if ($debug) {&message(".")};
	open(my $data, '<', $xmlfile)  || die "svcconfig.pl: Cannot open file ".$xmlfile.". Exiting.";
	open(my $out, '>', $tmpfile)  || die "svcconfig.pl: Cannot open file ".$tmpfile.". Exiting.";
   	if ($debug) {&message("Check XML file: ".$xmlfile)};
    while (my $line = <$data>) {
    	if ($debug) {print("*")};
    	#$line =~ s/[:^print:]//g;
    	$line =~ s{ ( &\# ( x[0-9A-Z]+ | [0-9]+ ) ; ) } {
		...
		foreach my $exclude ( @html_exclude_string ) {
			$line =~ s/$exclude/_/g;
		}
        print $out $line;
	}
...
sub ReadConfigFile {
	my $xmlfile = $dir."/"."svc.config.backup.xml.check";
...
sub InsertDataCli {
	...
	if ( $fullcfgflg ) {
		...
		if($debug) { &message("Insert Full Config Epoch ".$cfg_ref->{'fullcfg_epoch'}." to config data.")};
	} else {
...
sub PrintData {
	#
	if ($debug) {&message("Output file: ${out_conf_file}.tmp.")};
	...
	close(CONFOUT);
	if ($debug) {&message("Rename output file to: ${out_conf_file}.")};
	my $ret = system("$cmd_mv ${out_conf_file}.tmp ${out_conf_file}");
...
sub GetPortfcDataCli {
	...
	while ($line = <CMDOUT>) {
		...
		if ($line  =~ /^id${delim}fc_io_port_id${delim}.*$/) {   # header
			...
			my %record;
			@record{@header} = split(/$delim/,$line);
			if (length($record{node_id}) == 0){next;}
			...
			foreach my $m ( @{$n->{'property'}} ) {
				...
				if( $m->{'name'} eq "copy_id" ) {
					...
					if ( $m->{'value'} =~ /ssd/ ) {
						...
						&warning("Unknown tiername ".$m->{'value'}.".");
...
sub PrintPortfcData {
	...
	print CONFOUT "\nPort Level Configuration - Fibre Channel Ports\n";
	print CONFOUT   "-----------------------------------------------\n";
