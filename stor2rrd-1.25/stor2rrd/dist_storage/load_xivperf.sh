...
if [ `uname -a|grep AIX|wc -l` -eq 1 ]; then
  # wbemcli is compiled with openssl from /opt/freeware/lib so it must be here only for XIV storage
...
do
	...
	if [ ! $? -eq 0 ]; then
        ...
        # Remove old files
        find $XIV_DIR -name '*xivconf*data' -amin +60 -exec rm -f {} \;
        find $XIV_DIR -name '*xivconf*out' -atime +2 -exec rm -f {} \;
        find $XIV_DIR -name '*xivperf*out' -atime +30 -exec rm -f {} \;
done
