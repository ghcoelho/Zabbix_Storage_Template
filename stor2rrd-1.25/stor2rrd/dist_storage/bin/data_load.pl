...
sub load_storage
{
    ...
    if ( $st_type =~ m/^HUS$/ ) {
      ...
      if (! -l "$wrkdir/$storage/RG" ) {
        print "ln -s          : $wrkdir/$storage/RANK $wrkdir/$storage/RG\n" if $DEBUG ;
        symlink("$wrkdir/$storage/RANK","$wrkdir/$storage/RG") || die   "$act_time: Cannot ln -s $wrkdir/$storage/RG: $!";
      }
    }
    ...
    if ( $st_type =~ m/^HUS$/ ) {
      LoadDataModule::load_data_hus_all ($storage, $wrkdir, $webdir, $act_time, $STEP, $DEBUG, $no_time, $st_type);
    }
    if ( $st_type =~ m/^NETAPP$/ ) {
      LoadDataModule::load_data_netapp_all ($storage, $wrkdir, $webdir, $act_time, $STEP, $DEBUG, $no_time, $st_type);
    }
