...
TEST_LOG=`egrep "ERROR:" $ERRLOG | grep -v GPRINT | tail -5 | wc -l|sed 's/ //g'`
if [ "$TEST_LOG" = "0" ]; then
...
TEST_LOG=`egrep "ERROR:" $INPUTDIR/logs/error.log | grep -v GPRINT | tail -5 | wc -l|sed 's/ //g'`
if [ "$TEST_LOG" = "0" ]; then
...
TEST_LOG=`egrep "ERROR:" "$INPUTDIR/logs/error-cgi.log" | grep -v GPRINT | tail -5 | wc -l|sed 's/ //g'`
if [ "$TEST_LOG" = "0" ]; then
...
TEST_LOG=`egrep "lpar2rrd|stor2rrd" $APACHE_ERROR_LOG | tail -5 | wc -l|sed 's/ //g'`
if [ "$TEST_LOG" = "0" ]; then
...
TEST_LOG=`egrep "lpar2rrd|stor2rrd" $APACHE_ACCESS_LOG | tail -5 | wc -l|sed 's/ //g'`
if [ "$TEST_LOG" = "0" ]; then
...
TEST_LOG=`egrep "\" 4[0-9][0-9]" $APACHE_ACCESS_LOG | tail -5 | wc -l|sed 's/ //g'`
if [ "$TEST_LOG" = "0" ]; then
  echo "apache access log is OK"
fi
